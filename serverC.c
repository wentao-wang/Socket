...
#define PORT "23165"   // port number for ServerC
...
char* search (char input[]) {
	...
	if (Find(&listC, input) == NULL) {
		//printf("no find\n");
		return NULL;
	} else {
		char * res = Find(&listC, input)->define ;
		//printf("%s\n", res );
		return res;
	}
}
...
// loop through all the results and bind to the first we can----Beej
	...
	while(1) {
		...
		if (func == 1) {
			printf("The ServerC received input <%s> and operation <search> \n", input);
			...
			printf("The ServerC received input <%s> and operation <prefix> \n", input);
			int count = 0;
			char data[999];
			memset(data, '\0', 999*sizeof(char));
			ListElem* e = First(&listC);
			for (int i = 0; i < listC.num_members; i++) {
