...
#define PORT "22165"   // port number for ServerB
...
char* search (char input[]) {
	...
	if (Find(&listB, input) == NULL) {
		//printf("no find\n");
		return NULL;
	} else {
		char * res = Find(&listB, input)->define ;
		//printf("%s\n", res );
		return res;
	}
}
...
// loop through all the results and bind to the first we can----Beej
	...
	if (p == NULL) {
		fprintf(stderr, "serverB: failed to bind\n");
		return 0;
	}
	...
	while(1) {
		...
		if (func == 1) {
			printf("The ServerB received input <%s> and operation <search> \n", input);
			...
			printf("The ServerB received input <%s> and operation <prefix> \n", input);
			int count = 0;
			char data[999];
			memset(data, '\0', 999*sizeof(char));
			ListElem* e = First(&listB);
			for (int i = 0; i < listB.num_members; i++) {
